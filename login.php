...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? null)) {
        ...
        if ($user && $user['status'] === 'active' && password_verify($password, $user['password'])) {
            ...
            $_SESSION['user'] = $user;
            if ($user['role'] === 'admin') {
                try { $log = $pdo->prepare("INSERT INTO activity_logs(user_id,action,entity_type,entity_id,details) VALUES(?,?,?,?,?)"); $log->execute([(int)$user['id'],'Admin login','auth',(int)$user['id'],'Successful administrator login']); } catch (Throwable $e) {}
            }
